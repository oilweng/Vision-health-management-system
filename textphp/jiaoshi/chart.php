...
$sex1 = "男";
$sex2 = "女";
$sql1="select * from datadata where SE < '$aas' and age >= 5 and age <= 8 and sex = '$sex1'";
    $result_user1=mysqli_query($conn,$sql1);//执行对数据库的查询
    $rows1=mysqli_num_rows($result_user1);
$sql2="select * from datadata where SE < '$aas' and age >= 5 and age <= 8 and sex = '$sex2'";
    $result_user2=mysqli_query($conn,$sql2);//执行对数据库的查询
    $rows2=mysqli_num_rows($result_user2);
$sql3="select * from datadata where SE < '$aas' and age >= 9 and age <= 12 and sex = '$sex1'";
    $result_user3=mysqli_query($conn,$sql3);//执行对数据库的查询
    $rows3=mysqli_num_rows($result_user3);
$sql4="select * from datadata where SE < '$aas' and age >= 9 and age <= 12 and sex = '$sex2'";
    $result_user4=mysqli_query($conn,$sql4);//执行对数据库的查询
    $rows4=mysqli_num_rows($result_user4);
$sql5="select * from datadata where SE < '$aas' and age >= 13 and age <= 16 and sex = '$sex1'";
    $result_user5=mysqli_query($conn,$sql5);//执行对数据库的查询
    $rows5=mysqli_num_rows($result_user5);
$sql6="select * from datadata where SE < '$aas' and age >= 13 and age <= 16 and sex = '$sex2'";
...
   
        ...
        var option = {
            ...
            series: [{
                name: '男生',
                type: 'bar',
                data: [query1, query3, query5]
                            },
                            {
                name: '女生',
                type: 'bar',
                data: [query2, query4, query6]
                            }]
        };
